...
### Conversation identity: threads (Aaron's last three reports, shipped 2026-09-11)
- A GROUP created from ＋ New always mints a fresh THREAD (key "th-…") with its own identity — the same people can hold any number of separate threads. A single chat still merges into the existing 1:1. Thread membership lives on convMeta.participantIds (createdBy stamped); membership IS visibility: a member added later reads the WHOLE thread, and a removed member loses all of it.
- The 👥 header button opens the members panel: list with roles + "started it"; any member adds anyone they may message (added person gets the "whole thread is yours to read" notification); remove is creator/full_admin only, never the creator. Legacy conversations show the fixed-membership explainer instead — their behavior is UNCHANGED (participant-set key, per-message visibility, no threadId).
- @-mentions: typing "@" in the chat composer offers the thread's people; a mention sends the distinct "🔔 … mentioned you" notification (others get the normal ping), prefixes the email subject with 🔔, and renders highlighted in the bubble.
- Replies by a late joiner must land (the involved-check accepts current thread members). Reply-by-email tokens carry the thread key; /api/inbound-email resolves membership from convMeta for "th-" keys.
